...
export default function NewActivityTagForm({ onTagCreate }) {
  const { toast } = useToast();

  const NewActivityTagFormSchema = z.object({
    tag: z
      .string()
  ...
  const form = useForm({
    resolver: zodResolver(NewActivityTagFormSchema),
    defaultValues: {
      tag: "",
    },
  });
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormDescription>Enter activity tag.</FormDescription>
              <FormMessage />
            </FormItem>
          )}
        />
