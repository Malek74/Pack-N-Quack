...
fetchData();
  ...
  const resetForm = () => {
    setFormData({ title: '', description: '', price: '', language:'', accessibility:'', pickUpLocation:'', dropOffLocation:'', days:[], tags:'', dates: [],  direction: 'flex-row' });
    setDates([]);
    setActivitiesByDay([]);
    setSelectedTags([]);
    setShowModal(false);
  ...
  const handleAddActivity = (dayNumber, newActivity) => {
    ...
    const dayIndex = activitiesByDay.findIndex(daySearchIndex => daySearchIndex.day === parseInt(dayNumber));
  ...
  return (
    <div className='flex flex-col gap-y-10 py-8 px-[5.6rem]'>
      ...
      />
          ...
          <AddActivity className showModal={showActivityModal} setShowModal={setShowActivityModal} onSubmit={handleAddActivity}/>
      ...
      {itinerariesDB.map((itinerary, index) => (
        <ItineraryCard 
          ...
          onDelete={() => {handleDelete(itinerary._id) }} // Pass the delete function
          onUpdate={() => {openUpdateModal(index, itinerary._id)}} // Pass the update function
        />
      ))}
