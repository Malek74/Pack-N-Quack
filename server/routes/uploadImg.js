    ...
    try {
        ...
        for (const file of req.files) {
            ...
            const result = await new Promise((resolve, reject) => {
                const uploadStream = cloudinary.uploader.upload_stream(
                    {
                        resource_type: 'auto', // 'raw' for documents like PDFs
                        public_id: publicId, // Use the sanitized original file name
                        overwrite: true, // Optional: Set to true if you want to overwrite existing documents with the same public_id
                    },
                    (error, result) => {
                        if (error) {
                            return reject(error);
            ...
            documentUrls.push(result.secure_url); // Store the uploaded document URL
        }
